	...
	<div id="intro" class="<?php if($e404_options['blog_intro_type'] == 'twitter') echo 'twitter'; else echo 'text'; ?>-intro">
		...
		<?php else :
			if($e404_options['blog_intro_type'] == 'title' || $e404_options['blog_intro_type'] == 'title-excerpt') {
				the_post();
				if (is_day())
					printf('<h1>'.__('Daily Archives: <span>%s</span>', 'cold').'</h1>', get_the_date());
				elseif(is_month())
					printf('<h1>'.__('Monthly Archives: <span>%s</span>', 'cold').'</h1>', get_the_date('F Y'));
				elseif(is_year())
					printf('<h1>'.__('Yearly Archives: <span>%s</span>', 'cold').'</h1>', get_the_date('Y'));
				elseif(is_tag())
