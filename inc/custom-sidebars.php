...
function e404_add_options_panel() {
    add_submenu_page('themes.php', __('Manage Sidebars', 'cold'), __('Sidebars', 'cold'), 'level_8', 'custom_sidebars', 'e404_sidebars_options_page');
}
...
function e404_sidebars_options_page() {
    ...
    echo '<div class="wrap"><h2>'.__('Manage Sidebars', 'cold').'</h2>';
    ...
    if(isset($_POST['add_sidebar'])) {
        ...
        if(!empty($sidebar_name)) {
            ...
            if($index === false) {
                ...
                update_option('e404_custom_sidebars', $e404_custom_sidebars);
                echo '<div id="message" class="updated below-h2"><p>'.__('New sidebar added.', 'cold').'</p></div>';
            }
        }
    }
    ...
    if(isset($_GET['sidebar_id']) && $_GET['action'] == 'delete') {
        ...
        if(!empty($sidebar_name) && $sidebar_name != '(default)') {
            ...
            sort($e404_custom_sidebars);
            update_option('e404_custom_sidebars', $e404_custom_sidebars);
            echo '<div id="message" class="updated below-h2"><p>'.__('Sidebar removed.', 'cold').'</p></div>';
        }
    }
    
    echo '<table class="widefat page fixed" cellspacing="0">';
    echo '<thead><tr><th>'.__('Sidebar Name', 'cold').'</th><th>'.__('Action', 'cold').'</th></tr></thead>';
    echo '<tfoot><tr><th>'.__('Sidebar Name', 'cold').'</th><th>'.__('Action', 'cold').'</th></tr></tfoot><tbody>';
    foreach($e404_custom_sidebars as $sidebar) {
        echo '<tr><td>'.$sidebar.'</td><td>';
        if($sidebar != '(default)')
            echo '<a href="themes.php?page=custom_sidebars&action=delete&sidebar_id='.$sidebar.'">Delete</a>';
        else
            _e('The default sidebar can\'t be removed', 'cold');
        echo '</td></tr>';
    }
    echo '</tbody></table>';
    echo '<form method="post" action=""><div class="form-field"><label for="new_sidebar_name">'.__('Add new sidebar', 'cold').'</label> <input style="width: 200px" id="new_sidebar_name" name="new_sidebar_name" />';
...
function e404_add_category_sidebar_box($tag) {
    ...
    if(is_object($tag)) {
        ...
        echo '<tr class="form-field"><th><label for="e404_category_sidebar">'.__('Sidebar', 'cold').'</label></th><td><select name="e404_category_sidebar" class="postform" id="e404_category_sidebar">';
    }
    else {
        echo '<div class="form-field"><label for="e404_category_sidebar">'.__('Sidebar', 'cold').'</label><select name="e404_category_sidebar" class="postform" id="e404_category_sidebar">';
    }
    foreach($e404_custom_sidebars as $sidebar) {
    ...
    if($tag_ID) {
        echo '</select><br /><span class="description">'.__('Select a custom sidebar for this category or pick "default" for default blog sidebar.', 'cold').'</span></td></tr></tbody></table>';
    }
    else {
        echo '</select><p>'.__('Select a custom sidebar for this category or pick "default" for default blog sidebar.', 'cold').'</p></div>';
    }
}
