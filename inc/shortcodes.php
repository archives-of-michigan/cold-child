...
function e404_icon_box_shortcode($atts, $content = null) {
	extract(shortcode_atts(array('icon' => '', 'url' => '', 'title' => '', 'caption' => '', 'more_text' => ''), $atts));
	
	$output = '';
	$output .= '<h2 class="head-icon">';
	if(!empty($icon))
		$output .= '<img src="'.OF_DIRECTORY.'/images/icons/'.$icon.'.png" class="icon" alt="'.$title.'" />';
	if(empty($url))
		$output .= $title;
	else
	...
	if(!empty($url) && !empty($more_text)) {
		$output .= '<p class="more"><a href="'.$url.'">'.$more_text.'</a></p>';
	}
...
function e404_dropcap_shortcode($atts, $content = null) {
	extract(shortcode_atts(array('style' => '', 'color' => '', 'bgcolor' => ''), $atts));
	$styles = array('1', '2', '3', '4', '5', '6');
	if(empty($style) || !in_array($style, $styles))
		$style = '1';
	
	$before = '<span class="dropcap'.$style.'"'.e404_custom_colors_style($bgcolor, $color).'>';
	$after = '</span>';
...
function e404_button_shortcode($atts, $content = null) {
	...
	$custom_style = e404_custom_colors_style($bgcolor, $textcolor, $bordercolor);
	if(!empty($width)) {
		if(!empty($custom_style))
			$custom_style = str_replace(';"', '; '.$width.'"', $custom_style);
		else
			$custom_style = ' style="'.$width.'"';
	}
	
	$before = '[raw]<a class="'.$class_a.'"'.$target.' href="'.$href.'"><span'.$class_span.$custom_style.'>';
	if(!empty($icon))
...
// icons
$icon_types_normal = array('access-denied', 'alert', 'alert2', 'info', 'arrow-right', 'arrow-left', 'arrow-down', 'arrow-up', 'arrow', 'arrow2', 'checkmark', 'glass', 'plus', 'minus', 'user', 'help', 'bubble', 'bubbles', 'tag', 'download', 'calendar', 'clock', 'chart', 'cog', 'cd', 'document', 'folder', 'home', 'film', 'image', 'sound', 'link', 'key', 'locked', 'paperclip', 'marker', 'mail', 'rss');
...
function e404_image_shortcode($atts, $content = null) {
	...
	$image_size = array('vtiny' => 80, 'tiny' => 120, 'vsmall' => 152, 'small' => 200, 'medium' => 280, 'large' => 440, 'huge' => 600, 'full' => 920);
...
function e404_accordion_shortcode($atts, $content = null) {
	...
	$before .= '<div class="accordion_content"'.e404_custom_colors_style($bgcolor, $color, '', true).'><p>';
	$after = '</p></div>';
...
function e404_toggle_shortcode($atts, $content = null) {
	...
	$before .= '<span'.e404_custom_colors_style('', $header_color, '').'>'.$title.'</span></h4>';
	$before .= '<div class="toggle_content"'.e404_custom_colors_style($bgcolor, $color, '', true).'><p>';
	$after = '</p></div></div>';
...
//	return '<div class="tab_content" style="display: none;"><p>'.do_shortcode($content).'</p></div>';
	return '<div class="tab_content" style="position: absolute; left: -10000px;"><p>'.do_shortcode($content).'</p></div>';
}
...
function e404_pricebox_shortcode($atts, $content = null) {
	...
	$before = '<div class="pricebox'.$featured.'"><h3>'.$title.'</h3>';
	$after = '</div>';
...
function e404_recent_posts_shortcode($atts) {
	extract(shortcode_atts(array('thumbs' => '', 'number' => 5, 'title' => __('Recent Posts', 'cold'), 'limit' => 80, 'categories' => ''), $atts));
	...
	if ($r->have_posts()) :
		...
		while ($r->have_posts()) : $r->the_post();
			...
			$output .= '<li>';
			if($img)
				$output .= '<div><a href="'.get_permalink().'" class="recent-link" title="'.esc_attr(get_the_title() ? get_the_title() : get_the_ID()).'">'.$img.'</a></div>';
			
			$output .= '<h4><a href="'.get_permalink().'" class="recent-link" title="'.esc_attr(get_the_title() ? get_the_title() : get_the_ID()).'">'.get_the_title().'</a></h4>';
...
function e404_popular_posts_shortcode($atts) {
	extract(shortcode_atts(array('thumbs' => '', 'number' => 5, 'title' => __('Popular Posts', 'cold'), 'limit' => 80, 'categories' => ''), $atts));
	...
	if ($r->have_posts()) :
		...
		while ($r->have_posts()) : $r->the_post();
			...
			$output .= '<li>';
			if($img)
				$output .= '<div><a href="'.get_permalink().'" class="popular-link" title="'.esc_attr(get_the_title() ? get_the_title() : get_the_ID()).'">'.$img.'</a></div>';
			$output .= '<h4><a href="'.get_permalink().'" class="popular-link" title="'.esc_attr(get_the_title() ? get_the_title() : get_the_ID()).'">'.get_the_title().'</a></h4>';
...
function e404_tweets_shortcode($atts) {
	extract(shortcode_atts(array('number' => 5, 'title' => __('Last Tweets', 'cold'), 'time' => '', 'username' => ''), $atts));
	if(empty($username))
...
function e404_scrollable_shortcode($atts, $content = null) {
	...
	$output = '[raw]<div id="scroller"><a class="prev browse arrowleft"></a><div class="scrollable" id="'.$selector.'">';
	...
	foreach($images as $image) {
		if(trim($image) != '') {
			...
			$img_data = explode(";", trim($image));
			$output .= '<div><a href="'.$img_data[0].'" rel="prettyphoto[scrl'.$instance.']"><img src="'.e404_img_scale($img_data[0], 85, 85).'"';
			if(!empty($img_data[1]))
	...
	if(($i % 6) != 0)
		$output .= '</div>';
	$output .= '</div></div><a class="next browse arrowright"></a><br class="clear" /></div>';
...
function e404_nivo_shortcode($atts, $content = null) {
	extract(shortcode_atts(array('height' => '373', 'width' => '610', 'effect' => 'fade', 'slideshow' => '', 'buttons' => 'true', 'bubbles' => 'true', 'pause' => '2000', 'captions' => 'true'), $atts));
	...
	$args = array('post_type' => 'e404_slide', 'numberposts' => 99, 'e404_slideshow' => $slideshow, 'orderby' => 'menu_order date');
...
function e404_youtube_shortcode($atts) {
	extract(shortcode_atts(array('id' => '', 'height' => '373', 'width' => '610'), $atts));
	if(empty($id))
		return;
...
function e404_video_shortcode($atts) {
	extract(shortcode_atts(array('title' => '', 'poster' => '', 'height' => '373', 'width' => '610', 'source' => '', 'controls' => 'true'), $atts));
...
// [recent_works_images] shortcode
function e404_recent_works_images_shortcode($atts, $content = null) {
	extract(shortcode_atts(array('title' => __('Recent Works', 'cold'), 'url' => '', 'more_text' => __('view all', 'cold')), $atts));
	
	$before = '[raw]<div class="fancy_list">';
	
	$output = '<div class="one_fifth fancy_title"><h3>'.$title.'</h3>';
	if(!empty($url))
		$output .= '<p><a href="'.$url.'">'.$more_text.'</a></p>';
	$output .= '</div>';
	
	$query = array('post_type' => 'portfolio', 'orderby' => 'menu_order date', 'numberposts' => 4);
	...
	foreach($works as $work) {
		...
		$img_shortcode = '[image border="true" align="center" title="'.get_the_title($work->ID).'" shadow="true" size="vsmall" height="90"';
		$img_shortcode .= ' href="'.get_permalink($work->ID).'"]'.$large_image_url[0].'[/image]';
		if($i == $total)
			$output .= '<div class="one_fifth last fancy_list_item">'.$img_shortcode.'</div>';
		else
			$output .= '<div class="one_fifth fancy_list_item">'.$img_shortcode.'</div>';
	}
	
	$after = '<br class="clear" /></div>[/raw]';

	return $before.do_shortcode($output).$after;
}
add_shortcode('recent_works_images', 'e404_recent_works_images_shortcode');

// [recent_posts_images] shortcode
function e404_recent_posts_images_shortcode($atts) {
	extract(shortcode_atts(array('title' => __('Recent Blog Posts', 'cold'), 'url' => '', 'more_text' => __('view all', 'cold'), 'categories' => ''), $atts));
	...
	$query = array('orderby' => 'menu_order date', 'numberposts' => 4);
	if(!empty($categories))
	...
	foreach($posts as $post) {
		...
		if(!$large_image_url)
			$large_image_url[0] = OF_DIRECTORY.'/images/blank152.png';
		$img_shortcode = '<div class="shadow shadow_vsmall">
		<a href="'.get_permalink($post->ID).'"><img src="'.e404_img_scale($large_image_url[0], 152, 90).'" alt="'.get_the_title($post->ID).'" title="'.get_the_title($post->ID).'" class="border-img aligncenter" /></a>
		<div class="fancy_meta">
			<a href="'.get_comments_link($post->ID).'" class="fancy_comments">'.$post->comment_count.'</a>
		</div></div>';
		$img_shortcode .= '<h4><a href="'.get_permalink($post->ID).'">'.get_the_title($post->ID).'</a></h4>';
		if($i == $total)
			$output .= '<div class="one_fifth last fancy_list_item">'.$img_shortcode.'</div>';
		else
			$output .= '<div class="one_fifth fancy_list_item">'.$img_shortcode.'</div>';
