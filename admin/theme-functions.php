...
if (!function_exists('optionsframework_wp_head')) {
	function optionsframework_wp_head() {
		...
		if(!isset($_REQUEST['style']))
			if(isset($_COOKIE['cold_style']))
				$style = $_COOKIE['cold_style'];
			else
				$style = '';
		else 
	    ...
	    if ($style != '') {
	        ...
	        if($GLOBALS['stylesheet'] != '')
	            ...
	            echo '<link href="'. OF_DIRECTORY .'/styles/style_00.css" rel="stylesheet" type="text/css" />'."\n";         		  
	    }       
			
		if(empty($e404_all_options['e404_social_icons_variant']))
			$e404_all_options['e404_social_icons_variant'] = $e404_social_icons_variants[str_replace('.css', '', $GLOBALS['stylesheet'])];
...
function of_set_style_cookie() {
	if(isset($_REQUEST['style']) && !is_admin())
		setcookie('cold_style', $_REQUEST['style'], time() + 3600, '/');
}
