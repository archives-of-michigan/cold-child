...
if($custom_query->have_posts()) : while ($custom_query->have_posts()) : $custom_query->the_post(); $i++; ?>
				<div class="one_fourth portfolio-item<?php if($i % 4 == 0) echo ' last'; ?>" id="post-<?php the_ID(); ?>">
					<?php
					if (has_post_thumbnail()) {
						$large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'full');
						$img_shortcode = '[image border="true" shadow="true" align="none" title="'.the_title_attribute('echo=0').'" size="small"';
